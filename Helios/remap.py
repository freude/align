...
# numpy-only version of opencv's remap, which seems to do some strange
# truncation of values (8-bit assumptions, perhaps?)
...
def remap(src, warpcol, warprow, dest, repeat=False):
    if repeat:
        ...
        mask = (warpcol >= 0) & (warprow >= 0) & (warpcol <= src.shape[1] - 1) & (warprow <= src.shape[0] - 1)
